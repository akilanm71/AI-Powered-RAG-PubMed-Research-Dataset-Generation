...
class TrainingDataBot:
           ...
           async def load_dataset(self):
                   ...
                   return await self.database_manager.load_records()

           async def dataset_pipeline(self, source,
                                      task_types: Optional[List[TaskType]] = None, save_dataset: bool = False):
                ...
                if save_dataset:
                         ...
                         await self.database_manager.save_many(dataset.items)
           ...
           async def clear_pubmed(self):
                ...
                self.pubmed_service.incremental_manager.clear_ids()

                return {
                    "status": "success",
                    "message": "PubMed store and processed PMIDs cleared"}
           ...
           async def clear_dataset(self):

                        return await self.database_manager.clear_records()
...
async def main():
    ...
    await bot.dataset_pipeline(
        source = Path("data/"), 
        task_types=[TaskType.SUMMARIZATION]  ,save_dataset=True)
    ...
    await bot.database_manager.clear_records()

    print("Dataset Cleared")

    records = await bot.load_dataset()

    print(f"Records After Clear: {len(records)}")
